...
Under ETSI TS 119 472-3, providers shall support at least one of **`authorisation_code`** or **`urn:ietf:params:oauth:grant-type:pre-authorised_code`** in the Credential Offer. This RFC profiles both, and treats them as first-class issuance paths. PAR requirements apply only to the Authorisation Code Flow path.
...
2. The Wallet determines whether the selected issuance interaction is supported through:
    - **Authorisation Code Flow**;
...
3. Where this profile requires **Wallet Instance Attestation (WIA)** for the Authorisation Code Flow path, the Wallet includes:
   - the WIA at the PAR request; and
   - proof-of-possession of the key referenced in the WIA `cnf` claim, according to the profiled mechanism.
...
2. Where this profile requires **Wallet Instance Attestation (WIA)** for the Authorisation Code Flow path, the Wallet includes:
   - the WIA at the Token Endpoint; and
...
4. Where the selected proof method is based on `proofs.jwt`:
   ...
   - the Wallet carries the **Wallet Unit Attestation (WUA)** in the protected header parameter profiled for key attestation.
5. Where the selected proof method is based on `proofs.attestation`:
   - the Wallet carries the **Wallet Unit Attestation (WUA)** directly in the attestation structure; and
   - the Issuer processes the request according to the rules applicable to attested keys under the selected profile.
6. The Credential Issuer validates:
   - the access token and its sender-constraining mechanism;
   - the credential request syntax;
   - the nonce and proof material;
...
1. The Issuer constructs a Credential Offer that includes at minimum:
...
2. The Credential Offer MAY support either:
   - **Authorisation Code Flow**;
   - **Pre-Authorised Code Flow**; or
   - both.
...
1. Issuers and Wallets **SHALL** support both **same-device** and **cross-device** delivery of Credential Offers.
1. For **same-device**, implementations **SHALL** support at least the custom URL scheme `openid-credential-offer://`.
1. For **cross-device**, implementations **SHALL** support delivery through a QR code carrying a valid Credential Offer URI
...
2. The request includes:
   ...
   - Wallet Unit Attestation where required by the selected proof method or profile.
...
5. Support at least one of the following grant types in Credential Offers:
   - `authorization_code`;
...
4. Where WIA is required, validate:
   - the WIA signature under the Wallet Provider public key from the applicable trusted list;
   - the WIA expiry;
   - the proof-of-possession under the public key referenced in the WIA `cnf` claim.
...
5. ETSI TS 119 472-3 V1.1.1 (2025-02), *Electronic Signatures and Trust Infrastructures (ESI); eSignatures and Infrastructures (eIDAS) framework and ecosystem; Profiles for electronic attestation of attributes and implementations of person identification data; Part 3: European Digital Identity Wallet implementation profile*, <https://www.etsi.org/deliver/etsi_ts/119400_119499/11947203/01.01.01_60/ts_11947203v010101p.pdf>
