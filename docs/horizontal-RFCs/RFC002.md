...
The **<components:Verifier Backend>**, as defined in the APTITUDE glossary, is the server-side component that creates presentation requests, receives presentation responses, validates them, and returns the result to the relying application.
...
- the <roles:Verifier> creates a <artifacts:Presentation Request> using OpenID4VP-compatible request parameters
...
1. The <roles:Verifier> creates a <artifacts:Presentation Request>.
...
- the <roles:Verifier> performs ISO-bound validation of the `DeviceResponse`, including document type match, <data-elements:Namespace|namespace> and element consistency, <artifacts:Nonce|nonce> and handover consistency, and cryptographic validation against the reconstructed session context
...
The <roles:Verifier> prepares a <artifacts:Presentation Request> containing the information needed by the Wallet to determine what credentials and claims are requested, along with the parameters needed to produce a valid response.
...
The Wallet SHALL validate the received <artifacts:Presentation Request> before presenting it to the <roles:Holder>.
...
The <roles:Verifier> creates the <artifacts:Presentation Request> and makes it available through a non-API-mediated cross-device transfer mechanism, typically by encoding reference data or invocation data in a QR code or equivalent.

#### **6.2.2 Wallet Invocation via QR, Link, or Equivalent**

The <roles:Holder> uses a second device to invoke the Wallet and retrieve the <artifacts:Presentation Request>.
...
**APT-PRES-WALLET-05**  
Validate the <artifacts:Presentation Request> before presenting it to the <roles:Holder>.
...
| `VP-CHECK-11` | mdoc `DeviceResponse` validation | For ISO/IEC 18013-7 remote mdoc presentation, the <roles:Verifier> validates `DeviceResponse` presence and format, document type match, <data-elements:Namespace\|namespace> and element consistency, request-response correlation, <artifacts:Nonce\|nonce> and handover consistency, and cryptographic validation of the returned mdoc presentation object against the reconstructed `SessionTranscript`. | ARF credential-format interoperability; CIR presentation interface support; ISO/IEC TS 18013-7 alignment |
