...
2. The Wallet sends the request to the Issuer's PAR endpoint using the required client authentication method.
...
3. The Issuer returns the credential immediately, or signals deferred issuance.
...
2. Issuers and Wallets **MUST** support both **same-device** and **cross-device** delivery of Credential Offers.
3. For **same-device**, implementations **MUST** support at least the custom URL scheme `openid-credential-offer://`.
4. For **cross-device**, implementations **MUST** support delivery through a QR code carrying a valid Credential Offer URI
5. Additional wallet invocation mechanisms remain optional.
...
4. Batch requests MAY contain a mix of immediate and deferred items.
